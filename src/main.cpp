...
#if defined(NDEBUG)
# error "Frozenmist cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x114f202264cb42bf8378a6cef8c260c31f35e898fbed3ac1ad791131cb38e6e7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Frozenmist: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Frozenmist Signed Message:\n";
...
{
    // Frozenmist: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 20; // Frozenmist: 20 seconds
static const int64 nTargetSpacing = 1 * 20; // Frozenmist: 20 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Frozenmist: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Frozenmist: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x114f202264cb42bf8378a6cef8c260c31f35e898fbed3ac1ad791131cb38e6e7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Frozenmist is developed using blockchain technology.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1503687540;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2122331;
        ...
        assert(block.hashMerkleRoot == uint256("0x127d3be38064e31683be270d9296259352dc4ac33d343b9f7c05fc84227002b0"));
...
unsigned char pchMessageStart[4] = { 0x4b, 0xc4, 0xba, 0x6b }; // Frozenmist: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FrozenmistMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FrozenmistMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FrozenmistMiner(CWallet *pwallet)
{
    printf("FrozenmistMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("frozenmist-miner");
    ...
    try { loop {
        ...
        printf("Running FrozenmistMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FrozenmistMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FrozenmistMiner, pwallet));
}
